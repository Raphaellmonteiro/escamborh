...
export default function SolicitacaoModal({ isOpen, onClose }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError('');
    if (!form.segmento) { setError('Selecione o segmento da sua operação.'); return; }
    setLoading(true);
    try {
          ...
          >
            ...
            {step === 'success' ? (
              <div className="flex flex-col items-center justify-center px-4 py-10 text-center sm:px-6 sm:py-14">
                ...
                <h3 className="text-lg font-black text-white mb-2">Pedido recebido!</h3>
                <p className="text-sm mb-6" style={{ color: 'rgba(148,163,184,0.7)' }}>
                  Nosso time vai falar com você por WhatsApp ou e-mail para liberar seu teste e alinhar sua operação.
                </p>
              ...
              <form onSubmit={handleSubmit} className="space-y-4 px-4 pb-4 sm:space-y-5 sm:px-6 sm:pb-6">
                ...
                <motion.button
                  ...
                  {loading ? '⏳ Enviando...' : 'Quero testar no meu estabelecimento'}
                </motion.button>
