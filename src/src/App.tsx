...
const KDSPinGate            = lazy(() => import('./segments/restaurante/KDSPinGate'));
...
React.useEffect(() => {
  ...
  if (mesaMatch) {
    const mesaQrToken = new URLSearchParams(window.location.search).get('t') || undefined;
    if (!mesaQrToken) {
      return (
        <div style={{ minHeight: '100vh', background: '#09090b', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'sans-serif' }}>
          <div style={{ textAlign: 'center', color: '#71717a', padding: 32 }}>
            <div style={{ fontSize: 48, marginBottom: 16 }}>🚫</div>
            <h2 style={{ color: '#fff', marginBottom: 8 }}>QR Code inválido</h2>
            <p>Use o QR Code impresso na mesa para acessar o cardápio.</p>
          </div>
        </div>
      );
    }
    return (
      <Suspense fallback={<PublicRouteFallback />}>
        <ClienteMesaScreen slug={mesaMatch[1]} mesa={mesaMatch[2]} qrToken={mesaQrToken} />
      </Suspense>
    );
  }
  ...
  if (kdsSlug) {
    const savedToken = sessionStorage.getItem(`kds_token_${kdsSlug}`);
    const [kdsToken, setKdsToken] = React.useState<string | null>(savedToken);
    if (!kdsToken) {
      return (
        <Suspense fallback={<PublicRouteFallback />}>
          <KDSPinGate slug={kdsSlug} onUnlock={setKdsToken} />
        </Suspense>
      );
    }
    return (
      <Suspense fallback={<PublicRouteFallback />}>
        <KDSScreen slug={kdsSlug} kdsToken={kdsToken} />
      </Suspense>
    );
  }
...
function QRMesasModal({ slug, token, onClose }: { slug: string; token: string | null; onClose: () => void }) {
  const [mesas, setMesas] = React.useState<{ numero: number; qr_token?: string }[]>([]);
  ...
  React.useEffect(() => {
    (async () => {
      try {
        ...
        if (res.ok) {
          const data = await res.json();
          setMesas((data as any[]).map((m: any) => ({ numero: m.numero, qr_token: m.qr_token })));
        }
      } catch { /* sem mesas */ }
      setLoading(false);
    })();
  ...
  const mesaUrl = (num: number, qrToken?: string) => `${baseUrl}/mesa/${slug}/${num}${qrToken ? '?t=' + qrToken : ''}`;
        ...
        <div className="flex-1 overflow-y-auto p-8">
          {loading ? (
            ...
            <div className="grid gap-6" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(190px, 1fr))' }}>
              {mesas.map((m) => {
                const { numero } = m;
                const url = mesaUrl(m.numero, m.qr_token);
                return (
