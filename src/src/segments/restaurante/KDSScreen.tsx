...
export default function KDSScreen({ slug, kdsToken }: { slug: string; kdsToken?: string }) {
  ...
  const fetchOrders = React.useCallback(async () => {
    try {
      const res = await fetch(`/public/kds/${slug}`, { headers: kdsToken ? { 'X-Kiosk-Token': kdsToken } : {} });
  ...
  React.useEffect(() => {
    ...
    const connect = () => {
      if (closed) return;
      es = new EventSource(`/public/kds/${slug}/events${kdsToken ? '?kiosk_token=' + kdsToken : ''}`);  
  ...
  const advance = async (id: number) => {
    setAdvancing(id);
    try { await fetch(`/public/kds/${slug}/orders/${id}/advance`, { method: 'PATCH', headers: kdsToken ? { 'X-Kiosk-Token': kdsToken } : {} }); await fetchOrders(); }
