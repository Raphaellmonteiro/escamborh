...
export default function ClienteMesaScreen({ slug, mesa, qrToken }: { slug: string; mesa: string; qrToken?: string }) {
  ...
  const enviarPedido = async () => {
    ...
    if (!qrToken) { setErroPedido('QR Code inválido. Use o QR Code impresso na mesa.'); return; }
    setEnviando(true);
    setErroPedido('');
    try {
      const r = await fetch(`/public/mesa/${slug}/${mesa}/pedir`, {
        ...
        body: JSON.stringify({ itens: cart, qr_token: qrToken || '' }),
      });
