...
function forwardToCentralWhatsAppInbound(
  ...
  void registerInboundWhatsAppMessages({
    ...
    .then((result) => {
      ...
      if (result.accepted && result.savedCount > 0) {
        dispatchToN8NIfConfigured({
          tenantId: input.tenantId,
          event: 'message_received',
          payload: { event_name: eventName, instance: input.instance ?? null, provider: result.provider, saved_count: result.savedCount },
        });
      }
    })
    .catch((error) => {
