...
import { verifyStoredSecurityPassword } from '../utils/securityPasswordStorage';
...
document.addEventListener('keydown',e=>{if(e.key==='Enter'&&document.getElementById('tela-login').style.display!=='none')login();});
  ...
  // ── Display público (somente leitura, sem autenticação) ──────────────────
  router.get('/public/display/:slug', async (req, res) => {
    const slug = getPublicSlugOrReject(res, req.params.slug);
    if (!slug) return;
    try {
      const tenant = await q1('SELECT id, nome_estabelecimento FROM clientes WHERE usuario=?', [slug]);
      ...
      const allItems = await qAll(
        `SELECT i.order_id, i.quantity, p.name as product_name
         FROM itens_pedido i
      ...
      handlePublicRouteError(res, 'GET /public/display/:slug', slug, error);
    }
  });

  // ── KDS PIN auth ─────────────────────────────────────────────────────────
  router.post('/public/kds-pin/:slug', kioskRateLimit, async (req, res) => {
    const slug = getPublicSlugOrReject(res, req.params.slug);
    if (!slug) return;
    try {
      const { pin } = req.body;
      if (!pin || typeof pin !== 'string') return res.status(400).json({ error: 'PIN obrigatório' });
      const tenant = await q1('SELECT id, senha_admin FROM clientes WHERE usuario=?', [slug]);
      if (!tenant) return res.status(404).json({ error: 'Restaurante não encontrado' });
      const result = verifyStoredSecurityPassword(tenant.senha_admin, pin);
      if (!result.ok) return res.status(401).json({ error: 'PIN incorreto' });
      const secret = process.env.KIOSK_TOKEN_SECRET || JWT_SECRET;
      const token = crypto.createHmac('sha256', secret).update(`kds:${slug}`).digest('hex');
      return res.json({ token });
    } catch (error) {
      handlePublicRouteError(res, 'POST /public/kds-pin/:slug', slug, error);
    }
  });

  // ── KDS público ───────────────────────────────────────────────────────────
  router.get('/public/kds/:slug', requireKioskToken('kds'), async (req, res) => {
    const slug = getPublicSlugOrReject(res, req.params.slug);
    if (!slug) return;
  ...
  router.post('/public/mesa/:slug/:numero/pedir', requireBrowserOrigin, async (req, res) => {
    ...
    try {
      ...
      const qrToken = String(req.body.qr_token || req.query.qr_token || '').trim();
      if (!mesa.qr_token || !qrToken || qrToken !== mesa.qr_token) {
        return res.status(403).json({ error: 'QR Code inválido. Use o QR Code impresso na mesa.' });
      }
